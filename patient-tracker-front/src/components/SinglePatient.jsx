...
const SinglePatient = () => {
  ...
  const patient = patientsData.find(patient => patient.id === parseInt(id));
  if (!patient) {
    ...
    <button className="mb-4 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={handleGoBack}> Go Back</button>
      ...
      <div>
        ...
        <div>
          ...
          <div>
            <h4 className="text-lg font-bold mb-2">Appointments</h4>
            {patient.appointments.map((appointment, index) => (
              <div key={index}>
                <p>Date: {appointment.date}</p>
                <p>Purpose: {appointment.purpose}</p>
                <p>Attending Physician: {appointment.attendingPhysician}</p>
              </div>
            ))}
          </div>
          <div>
